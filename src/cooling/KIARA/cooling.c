...
__attribute__((always_inline)) INLINE float cooling_compute_G0(
    ...
    const struct cooling_function_data *cooling, const double dt) {
...
void cooling_copy_to_grackle2(
  ...
  if (cooling->use_grackle_dust_evol == 1) {
    ...
    species_densities[21] = p->feedback_data.SNe_ThisTimeStep * dt / p->mass *
...
void cooling_cool_part(const struct phys_const *restrict phys_const,
   ...
   * KIARA can't use this because the dust needs to be formed/destroyed over
  dt_therm if (time - xp->cooling_data.time_last_event < cooling->thermal_time)
  { return;
  }*/
...
void cooling_set_particle_subgrid_properties(
  ...
  if (T_floor > 0 && u < u_floor * cooling->entropy_floor_margin) {
    ...
    p->cooling_data.subgrid_dens = cooling_compute_subgrid_density(
        rho, temperature, p->cooling_data.subgrid_temp, floor_props, cooling);
  } else {
