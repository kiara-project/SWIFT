  ...
  switch (dir_flag) {
    ...
    case 0:
    {
      const double random_for_theta =
          random_unit_interval(si->id, ti_current, random_number_isotropic_SNII_feedback_ray_theta);
      const double random_for_phi =
          random_unit_interval(si->id, ti_current, random_number_isotropic_SNII_feedback_ray_phi);
    ...
    case 1:
    {
      ...
      random_number =
            random_unit_interval(si->id, ti_current, random_number_stellar_feedback_1);
...
//#ifdef KIARA_DEBUG_CHECKS
  message("KICK_PROB: z=%g sid=%lld, gid=%lld, prob=%g, rand=%g, eta=%g, mlaunch=%g, "
    ...
    */
    ...
    if (norm <= 0.f) {
      warning("z=%g sid=%lld pid=%lld normalization of wind direction is zero!\n(x, y, z) "
          "= (%g, %g, %g); vw=%g. Randomizing direction.", cosmo->z, si->id, pj->id,
...
/* Collect information about galaxy that the particle belongs to */
    ...
    printf("WIND_LOG z=%.5f sid=%lld mlaunch=%g mkicked=%g Nkicked=%g zbirth=%g M*=%g sSFR=%g pid=%lld vw=%g vwx=%g vwy=%g vwz=%g h=%g x=%g "
           ...
           "T=%g nH=%g tdel=%g Ndec=%d fZ=%g\n",
            ...
            galaxy_ssfr / fb_props->time_to_yr,
            pj->id,
