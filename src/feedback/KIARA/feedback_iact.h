...
runner_iact_nonsym_feedback_prep1(const float r2, const float dx[3],
  ...
  message(
      "STAR_PROB: sid=%lld, gid=%lld, prob=%g, eta=%g, mlaunch=%g, "
      "m*=%g, N_to_launch=%g, wj=%g, wt_sum=%g",
      si->id, pj->id, prob, si->feedback_data.mass_to_launch / si->mass_init,
      si->feedback_data.mass_to_launch, si->mass_init, N_to_launch, wj,
...
__attribute__((always_inline)) INLINE static void feedback_kick_gas_around_star(
  ...
  if (pj->feedback_data.kick_id == si->id) {
    const float rand_for_kick_dir = random_unit_interval(
        pj->id, ti_current, random_number_stellar_feedback);
    float kick_dir = 1.f;
     ...
     */
    const float wind_velocity = kick_dir * si->feedback_data.wind_velocity;
    ...
    if (!fb_props->use_firehose_model) {
      float galaxy_stellar_mass = pj->galaxy_data.stellar_mass;
    ...
    printf(
        ...
        pj->galaxy_data.stellar_mass * fb_props->mass_to_solar_mass,
        pj->galaxy_data.ssfr / fb_props->time_to_yr, pj->id,
