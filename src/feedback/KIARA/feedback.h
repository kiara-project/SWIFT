...
__attribute__((always_inline)) INLINE static int feedback_is_active(
  ...
  //message("FEEDBACK_IS_ACTIVE %d %g %d yes? %d", e->step, sp->birth_time, sp->count_since_last_enrichment, e->step <= 0 ||
...
feedback_compute_kick_velocity(const float galaxy_stellar_mass, 
    const size_t sp_id, const struct cosmology* cosmo,
    const struct feedback_props *fb_props, const integertime_t ti_current) {

  /* Compute galaxy mass. This is done in the RUNNER files. */
  float galaxy_stellar_mass_Msun = galaxy_stellar_mass;  // in code units for now
  ...
  const float rand_for_scatter = 
      random_unit_interval(sp_id, ti_current, 
...
__attribute__((always_inline)) INLINE static void feedback_prepare_feedback(
  ...
  float v_internal = 
      feedback_compute_kick_velocity(M_star, sp->id, cosmo, feedback_props, ti_begin);
  ...
  if (total_mass_kicked < wind_mass) {
    ...
    if (sp->galaxy_data.stellar_mass > 0.f && sp->galaxy_data.specific_sfr > 0.f &&
