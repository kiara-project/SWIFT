...
__attribute__((always_inline)) INLINE static double feedback_wind_probability(
  ...
  *rand_for_sf_wind =
  ...
  if (fb_props->early_wind_suppression_enabled) {
    const double early_minimum_stellar_mass =
        ...
        exp(-1. * ((cosmo->a / fb_props->early_wind_suppression_scale_factor) *
  ...
  *wind_mass = fb_props->FIRE_eta_normalization * stellar_mass_this_step;
  if (galaxy_stellar_mass < fb_props->FIRE_eta_break) {
    (*wind_mass) *= pow(galaxy_stellar_mass / fb_props->FIRE_eta_break,
  ...
  if (fb_props->early_wind_suppression_enabled) {
    if (cosmo->a < fb_props->early_wind_suppression_scale_factor) {
      (*wind_mass) *=
          pow(cosmo->a / fb_props->early_wind_suppression_scale_factor,
...
feedback_kick_and_decouple_part(struct part *p, struct xpart *xp,
  ...
  const double galaxy_stellar_mass = p->galaxy_data.stellar_mass;
  ...
  if (u_new / u_init > 1000) {
    warning(
        "Wind heating too large! T0=%g Tnew=%g fw=%g hwf=%g "
        " TSN=%g Tw=%g vw=%g ms=%g mwind=%g",
        u_init / fb_props->temp_to_u_factor, u_new / fb_props->temp_to_u_factor,
        f_warm, hot_wind_fraction, u_SN / fb_props->temp_to_u_factor,
        u_wind / fb_props->temp_to_u_factor, wind_velocity,
        p->sf_data.SFR * dt_part, wind_mass);
  ...
  const double dir[3] = {p->gpart->a_grav[1] * p->gpart->v_full[2] -
