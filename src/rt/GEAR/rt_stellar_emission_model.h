...
/*
 * @brief compute the index of a lower bound bin for a non-uniform spaced 1D
 * array_x. It takes an array_x that indicate the positions, its size and a
 * value x for which we wish to find the lower bound. It returns only the index
 * of the lower bound bin.
 *
 *
 * @param array_x Values of gridpoints
 * @param size    Length of array_x
 * @param x       Value within range of array_x for which we want to find the
 * lower bound
 */
static int interpolate_1D_non_uniform_LowerBound(const double *array_x,
  ...
  if (x <= array_x[0]) return 0;

  // Handle above bounds explicitly
  if (x >= array_x[size - 1]) return size - 2;

  // Find the lower bin: array_x[i] <= x < array_x[i+1]
  for (int i = 0; i < size - 1; ++i) {
    if (x < array_x[i + 1]) return i;
  }
...
/*
 * @brief compute an interpolated value from a 2d table (array_y) whose values
 * are given on a non-uniformly spaced grids. The first dimension uses the outer
 * bounds of the grid when given values below/above their min/max. The second
 * dimension does the same for its max but gives an error when below their min.
 *
 * @param array_x1 Values of gridpoints over first dimension
 * @param array_x2 Values of gridpoints over second dimension
 * @param array_y  Values of a 2D table that we interpolate, lengths must match
 * size1 and size2
...
static double interpolate_2D_non_uniform(const double *array_x1,
                                         const double *array_x2,
                                         double **array_y, const int size1,
                                         const int size2, double x1,
                                         double x2) {

  // First we check the bounds and if x1 and x2 are within it
  // return 0 if Time is above star age threshold, give error when below lowest
  // bin value (should always be 0 though if properly binned) check for star age
  ...
  // We know the values we need are at [Ind1,Ind2], [Ind1 + 1,Ind2], [Ind1,Ind2
  // + 1] and [Ind1 + 1,Ind2 + 1] We calculate the offsets over the first
  // dimension
  const double offset1 = (array_x1[Ind1 + 1] - x1) / dx1;
  // And now the second dimension
  const double offset2 = (array_x2[Ind2 + 1] - x2) / dx2;

  // First Interpolation:
  // Interpolate [Ind1,Ind2] --- [Ind1+1,Ind2]   and interpolate [Ind1,Ind2+1]
  // --- [Ind1+1,Ind2+1] with offset1 to make 2 new values.
  ...
  const double Interpol1 =
      offset1 * array_y[Ind1][Ind2] + (1. - offset1) * array_y[Ind1 + 1][Ind2];
  const double Interpol2 = offset1 * array_y[Ind1][Ind2 + 1] +
                           (1. - offset1) * array_y[Ind1 + 1][Ind2 + 1];
  ...
  if (isnan(result) || isinf(result)) {
    ...
    printf("array_y values: %g %g %g %g\n", array_y[Ind1][Ind2],
           array_y[Ind1 + 1][Ind2], array_y[Ind1][Ind2 + 1],
           array_y[Ind1 + 1][Ind2 + 1]);
    exit(1);
  }
 ...
 * @param ionizing_tables BPASS table that stores the total emitted photon
 * number at different stellar age
 * @param average_photon_energy average photon energy in each frequency bin, in
 * erg
 * @param phys_const struct holding physical constants
 * @param internal_units units struct containing internal units
 */
__attribute__((always_inline)) INLINE static void
rt_get_emission_this_step_BPASS(double emission_this_step[RT_NGROUPS], float M,
                                float Metallicity,
  ...
  static const double Metallicities[] = {
      1e-5, 1e-4, 0.01, 0.02, 0.03, 0.04, 0.06, 0.08, 0.1, 0.14, 0.2, 0.3, 0.4};
  ...
  for (int g = 0; g < RT_NGROUPS; g++) {
    if (star_age_before_Myr > 100.) {
      /* If the stellar age before this timestep is above 100Myr, we set the
       * emission to zero*/
      emission_this_step[g] = 0.;
    } else {
      if (!ionizing_tables[g]) error("ionizing_tables[%d] is NULL", g);
      for (int i = 0; i < size_Metals; i++) {
        if (!ionizing_tables[g][i])
          error("ionizing_tables[%d][%d] is NULL", g, i);
      }
      double N_total_before = interpolate_2D_non_uniform(
          Metallicities, age_100Myr, ionizing_tables[g], size_Metals,
          size_Times, Metallicity, star_age_before_Myr);
      double N_total_now = interpolate_2D_non_uniform(
          Metallicities, age_100Myr, ionizing_tables[g], size_Metals,
          size_Times, Metallicity, star_age_now_Myr);
      double N_emission_this_step = N_total_now - N_total_before;

      /* average photon densities are in cgs! */
      const double E_g = f_esc * average_photon_energy[g] *
                         N_emission_this_step * M_star_fraction / energy_units;
      emission_this_step[g] = E_g;

      if (E_g < 0) {
        error(
            "Negative Photons??, N_total_before: %e, N_total_now: "
            "%e,star_age_before_Myr: %e, star_age_now_Myr: %e",
            N_total_before, N_total_now, star_age_before_Myr, star_age_now_Myr);
      }

      // message("energy this step: %e, N_total_before: %e, N_total_now: %e,
      // average_photon_energy[g]: %e, star_age_before_Myr: %e,
      // star_age_now_Myr: %e, M_star_fraction: %e, N_emission_this_step:%e,
      // Metalicities: %e", E_g, N_total_before, N_total_now,
      // average_photon_energy[g], star_age_before_Myr, star_age_now_Myr,
      // M_star_fraction, N_emission_this_step, Metallicity);
    }
  }
}
