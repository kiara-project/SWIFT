 ...
 *
 ******************************************************************************/
#ifndef SWIFT_RT_PROPERTIES_KIARA_H
#define SWIFT_RT_PROPERTIES_KIARA_H
...
/**
 * @file src/rt/KIARA/rt_properties.h
 * @brief Main header file for the 'KIARA' radiative transfer scheme
 * properties.
 */

#define RT_IMPLEMENTATION "KIARA M1closure"
...
/**
 * @brief Properties of the 'KIARA' radiative transfer model
 */
struct rt_props {
...
__attribute__((always_inline)) INLINE static void rt_props_init(
  ...
  if (RT_NGROUPS <= 0) {
    error(
        "You need to run KIARA-RT with at least 1 photon group, "
        "you have %d",
        RT_NGROUPS);
  } else {
    /* !! Keep the frequencies in Hz for now. !! */
    parser_get_param_float_array(params, "KIARART:photon_groups_Hz", RT_NGROUPS,
                                 rtp->photon_groups);
  }
  ...
  parser_get_param_string(params, "KIARART:stellar_luminosity_model",
                          stellar_model_str);
  ...
  if (rtp->stellar_emission_model == rt_stellar_emission_model_const) {
    ...
    parser_get_param_double_array(params,
                                  "KIARART:const_stellar_luminosities_LSol",
    ...
    /* Nothing to do here */
             ...
             rt_stellar_emission_model_BPASS) {
    parser_get_param_string(params, "KIARART:stellar_table_path",
  ...
  /* --------------------------------- */
  const float f_r = parser_get_param_float(params, "KIARART:f_reduce_c");
  if (f_r <= 0.f)
  ...
  /* get CFL condition */
  /* ----------------- */
  const float CFL = parser_get_param_float(params, "KIARART:CFL_condition");
  ...
  const float f_limit_cooling_time = parser_get_opt_param_float(
      params, "KIARART:f_limit_cooling_time", /*default=*/0.6);
  ...
  rtp->hydrogen_mass_fraction =
      parser_get_param_float(params, "KIARART:hydrogen_mass_fraction");
  ...
  rtp->set_initial_ionization_mass_fractions = parser_get_opt_param_int(
      params, "KIARART:set_initial_ionization_mass_fractions",
      /* default = */ 0);
  if (rtp->set_initial_ionization_mass_fractions) {
    /* Read in mass fractions */
    rtp->mass_fraction_HI_init =
        parser_get_param_float(params, "KIARART:mass_fraction_HI");
    rtp->mass_fraction_HII_init =
        parser_get_param_float(params, "KIARART:mass_fraction_HII");
    rtp->mass_fraction_HeI_init =
        parser_get_param_float(params, "KIARART:mass_fraction_HeI");
    rtp->mass_fraction_HeII_init =
        parser_get_param_float(params, "KIARART:mass_fraction_HeII");
    rtp->mass_fraction_HeIII_init =
        parser_get_param_float(params, "KIARART:mass_fraction_HeIII");
  ...
  rtp->set_equilibrium_initial_ionization_mass_fractions =
      parser_get_opt_param_int(
          params, "KIARART:set_equilibrium_initial_ionization_mass_fractions",
          /* default = */ 0);
  ...
  rtp->stellar_spectrum_type =
      parser_get_param_int(params, "KIARART:stellar_spectrum_type");
  ...
  if (rtp->stellar_spectrum_type == 0) {
    ...
    rtp->const_stellar_spectrum_max_frequency = parser_get_param_float(
        params, "KIARART:stellar_spectrum_const_max_frequency_Hz");
    ...
    rtp->stellar_spectrum_blackbody_T = parser_get_param_float(
        params, "KIARART:stellar_spectrum_blackbody_temperature_K");
    ...
    rtp->f_esc = parser_get_param_float(
        params, "KIARART:photon_escape_fraction");
  ...
  rtp->stars_max_timestep = parser_get_opt_param_float(
      params, "KIARART:stars_max_timestep", /*default=*/FLT_MAX);
  ...
  /* ------------- */
  rtp->grackle_verbose =
      parser_get_opt_param_int(params, "KIARART:grackle_verbose", /*default=*/0);
  rtp->case_B_recombination = parser_get_opt_param_int(
      params, "KIARART:case_B_recombination", /*default=*/1);
...
#endif /* SWIFT_RT_PROPERTIES_KIARA_H */
